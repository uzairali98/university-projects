...
{
	for(int a=0,b,least;a<size-1;a++)
	{
		for(b=a+1,least=a;b<size;b++)
		{	
		  if(arr[b]<arr[least])
		    {
		      	least=b;
...
ofstream out;
out.open("selectionsort result.txt",ios::out);

cout<<"\t\t\t\t\t\tSELECTION SORT BENCHMARK"<<endl;
cout<<"\t\t\t\t\t\t\t(THREADS)"<<endl<<endl;
out<<"\t\t\t\t\t\tSELECTION SORT BENCHMARK"<<endl;
out<<"\t\t\t\t\t\t\t(THREADS)"<<endl<<endl;

cout<<"\nNOTE: Terminal only support limited no of lines so we will write the output to the output text file  'selectionsort result.txt'"<<endl;
int max_size=400000;
int sub_size=max_size/4;
cout<<"\n### Creating test array of size "<<max_size;
out<<"\n### Creating test array of size "<<max_size;
//cout<<"\n###NOTE: selection sort is very efficient that is why we decided to set very large array size which is "<<max_size;
...
pid_t p1,p2;
	...
	if (p1 > 0 && p2 > 0) 
	{
		...
		cout<<endl<<endl;
		cout<<"\n### writing output arrray to file radixsort result.txt";
		out<<"\n### Now printing sorted array:"<<endl;
		sleep(3);
		for(int a=0;a<max_size;a++)
		out<<"\narray["<<a<<"]-->"<<test_data[a];
		cout<<"\n### Writing to file is now finished please open selectionsort result.txt to view the complete result of this benchmark"<<endl;
	...
	else if (p1 == 0 && p2 > 0)
	{
		cout<<"\n### 2nd Process Running";
		out<<"\n### 2nd Process Running";
		selectionsort(arr2,sub_size);
	}
	else if (p1 > 0 && p2 == 0)
	{
		cout<<"\n### 3rd Process Running";
		out<<"\n### 3rd Process Running";
		selectionsort(arr3,sub_size);
	}
	else if (p1 == 0 && p2 == 0)
	{
		cout<<"\n### 4th Process Running";
		out<<"\n### 4th Process Running";
		selectionsort(arr4,sub_size);
	}
	else
	{
		cout<<"\n### Process Failed";
