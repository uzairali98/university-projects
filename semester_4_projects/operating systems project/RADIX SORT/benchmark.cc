...
{
  int maxdig=noofdigits(maxno(arr,size));
  queue <int> q[10];
  for(int i=0,x=1;i<maxdig;x*=10,i++)
  {
  	for(int j=0;j<size;j++)
  	{
  		q[(arr[j]/x)%10].push(arr[j]);
	}
	int k=0;
	for(int j=0;j<10;j++)
	{
...
ofstream out;
out.open("radixsort result.txt",ios::out);

cout<<"\t\t\t\t\t\tRADIX SORT BENCHMARK"<<endl;
cout<<"\t\t\t\t\t\t\t(THREADS)"<<endl<<endl;
out<<"\t\t\t\t\t\tRADIX SORT BENCHMARK"<<endl;
out<<"\t\t\t\t\t\t\t(THREADS)"<<endl<<endl;

cout<<"\nNOTE: Terminal only support limited no of lines so we will write the output to the output text file  'radixsort result.txt'"<<endl;
int max_size=1600000;
int sub_size=max_size/4;
cout<<"\n### Creating test array of size "<<max_size;
out<<"\n### Creating test array of size "<<max_size;
cout<<"\n###NOTE: radix sort is very efficient that is why we decided to set very large array size which is "<<max_size;
...
pid_t p1,p2;
	...
	if (p1 > 0 && p2 > 0) 
	{
		...
		for(int a=0;a<max_size;a++)
		out<<"\narray["<<a<<"]-->"<<test_data[a];
		cout<<"\n### Writing to file is now finished please open radixsort result.txt to view the complete result of this benchmark"<<endl;
	...
	else if (p1 == 0 && p2 > 0)
	{
		cout<<"\n### 2nd Process Running";
		out<<"\n### 2nd Process Running";
		radixsort(arr2,sub_size);
	}
	else if (p1 > 0 && p2 == 0)
	{
		cout<<"\n### 3rd Process Running";
		out<<"\n### 3rd Process Running";
		radixsort(arr3,sub_size);
	}
	else if (p1 == 0 && p2 == 0)
	{
		cout<<"\n### 4th Process Running";
		out<<"\n### 4th Process Running";
		radixsort(arr4,sub_size);
	}
	else
	{
		cout<<"\n### Process Failed";
