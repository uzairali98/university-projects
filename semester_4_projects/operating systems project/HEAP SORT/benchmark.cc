...
void heapit(int arr[],int hpsize,int i)
{
	int largest=i;
	int lc=2*i+1;
	int rc=2*i+2;
	
	if(lc<hpsize && arr[lc]>arr[largest])
	largest=lc;
	
	if(rc<hpsize && arr[rc]>arr[largest])
	largest=rc;
...
ofstream out;
out.open("heapsort result.txt",ios::out);

cout<<"\t\t\t\t\t\tHEAP SORT BENCHMARK"<<endl;
cout<<"\t\t\t\t\t\t\t(THREADS)"<<endl<<endl;
out<<"\t\t\t\t\t\tHEAP SORT BENCHMARK"<<endl;
out<<"\t\t\t\t\t\t\t(THREADS)"<<endl<<endl;

cout<<"\nNOTE: Terminal only support limited no of lines so we will write the output to the output text file  'heapsort result.txt'"<<endl;
int max_size=1600000;
int sub_size=max_size/4;
cout<<"\n### Creating test array of size "<<max_size;
out<<"\n### Creating test array of size "<<max_size;
cout<<"\n###NOTE: heapsort sort is very efficient that is why we decided to set very large array size which is "<<max_size;
...
{
 test_data[a]=rand()%max_size*2; //filling array with random data
}
...
pid_t p1,p2;
	...
	if (p1 > 0 && p2 > 0) 
	{
		...
		cout<<endl<<endl;
		cout<<"\n### writing output arrray to file heapsort result.txt";
		out<<"\n### Now printing sorted array:"<<endl;
		sleep(3);
		for(int a=0;a<max_size;a++)
		out<<"\narray["<<a<<"]-->"<<test_data[a];
		cout<<"\n### Writing to file is now finished please open heapsort result.txt to view the complete result of this benchmark"<<endl;
	...
	else if (p1 == 0 && p2 > 0)
	{
		cout<<"\n### 2nd Process Running";
		out<<"\n### 2nd Process Running";
		heapsort(arr2,sub_size);
	}
	else if (p1 > 0 && p2 == 0)
	{
		cout<<"\n### 3rd Process Running";
		out<<"\n### 3rd Process Running";
		heapsort(arr3,sub_size);
	}
	else if (p1 == 0 && p2 == 0)
	{
		cout<<"\n### 4th Process Running";
		out<<"\n### 4th Process Running";
		heapsort(arr4,sub_size);
	}
	else
	{
		cout<<"\n### Process Failed";
