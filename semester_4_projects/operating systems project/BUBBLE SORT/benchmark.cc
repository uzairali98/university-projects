...
ofstream out;
out.open("bubblesort result.txt",ios::out);

cout<<"\t\t\t\t\t\tBUBBLE SORT BENCHMARK"<<endl;
cout<<"\t\t\t\t\t\t\t(THREADS)"<<endl<<endl;
out<<"\t\t\t\t\t\tBUBBLE SORT BENCHMARK"<<endl;
out<<"\t\t\t\t\t\t\t(THREADS)"<<endl<<endl;

cout<<"\nNOTE: Terminal only support limited no of lines so we will write the output to the output text file  'bubblesort result.txt'"<<endl;
int max_size=400000;
...
{
 test_data[a]=rand(); //filling array with random data
}
...
pid_t p1,p2;
	...
	if (p1 > 0 && p2 > 0)
	{
		...
		for(int a=0;a<max_size;a++)
		{
		 if(ar1[i]<=ar2[j] && ar1[i]<=ar3[k] && ar1[i]<=ar4[l])
		 {
		  test_data[a]=ar1[i++];
		 }
		 else if(ar2[j]<=ar1[i] && ar2[j]<=ar3[k] && ar2[j]<=ar4[l])
		 {
		  test_data[a]=ar2[j++];
		 }
		 else if(ar3[k]<=ar1[i] && ar3[k]<=ar2[j] && ar3[k]<=ar4[l])
		 {
		  test_data[a]=ar3[k++];
		 }
		 else
		...
		cout<<endl<<endl;
		cout<<"\n### writing output arrray to file bubblesort result.txt";
		out<<"\n### Now printing sorted array:"<<endl;
		sleep(3);
		for(int a=0;a<max_size;a++)
		out<<"\narray["<<a<<"]-->"<<test_data[a];
		cout<<"\n### Writing to file is now finished please open bubblesort result.txt to view the complete result of this benchmark"<<endl;
	...
	else if (p1 == 0 && p2 > 0)
	{
		cout<<"\n### 2nd Process Running";
		out<<"\n### 2nd Process Running";
		bubble(arr2,sub_size);
	}
	else if (p1 > 0 && p2 == 0)
	{
		cout<<"\n### 3rd Process Running";
		out<<"\n### 3rd Process Running";
		bubble(arr3,sub_size);
	}
	else if (p1 == 0 && p2 == 0)
	{
		cout<<"\n### 4th Process Running";
		out<<"\n### 4th Process Running";
		bubble(arr4,sub_size);
	}
	else
	{
		cout<<"\n### Process Failed";
