...
int main ()
{
	...
	start:
		clean();makeborder();
	printf("\t\t         %s\n\t\t\t\t welcome\n",tm);
	printf("\npress 1 for loction on the plane "
	...
	{
		case(1):
			printf("enter value of x & y:"); fflush(stdin);
			scanf("%f %f",&x1,&y1);
			if((round(x1))>=0 && (round(y1)>=0)){quad=1;printf("point lies in 1st quadrant");fprintf(logf,"point lies in 1st quadrant",x1,y1);}
			else if((round(x1))<=0 && (round(y1)>=0)){quad=2;printf("point lies in 2nd quadrant");fprintf(logf,"point lies in 2nd quadrant",x1,y1);}
			else if((round(x1))<=0 && (round(y1)<=0)){quad=3;printf("point lies in 3rd quadrant");fprintf(logf,"point lies in 3rd quadrant",x1,y1);}
			else {quad=4;printf("point lies in 4th quadrant");fprintf(logf,"point lies in 4th quadrant",x1,y1);}
			fprintf(logf,"\nx=%0.2f y=%0.2f",x1,y1);
		...
		case(2):
			...
			printf("enter value of x1 y1 x2 y2:");scanf("%f%f%f%f",&x1,&y1,&x2,&y2);
			printf("distance is %0.2f",dis(x1,y1,x2,y2));
			...
			break;
		case(3):
			printf("enter value of x1 y1 x2 y2 ");scanf("%f%f%f%f",&x1,&y1,&x2,&y2);intptr2=midpoint(x1,y1,x2,y2);
			printf("midpoint is (%d,%d)",intptr2[0],intptr2[1]);
			...
			break;
		case(4):
		printf("enter value of distance and coordinates x1: y1: x2: y2:");scanf("%d%f%f%f%f",&dist,&x1,&y1,&x2,&y2);
		if (dist==(round(dis(x1,y1,x2,y2))))
		{
		printf("these points A(%0.2f,%0.2f) and B(%0.2f,%0.2f) verify this distance of %d units",x1,y1,x2,y2,dist);
		fprintf(logf,"these points A(%0.2f,%0.2f) and B(%0.2f,%0.2f) verify this distance of %d units",x1,y1,x2,y2,dist);
	}
		else
		    {
		printf("these points do not verify the given distance of %d units",dist);
		...
		if(x1==x2)
		{
		   ...
		   case(8):
			...
			case(13):
				...
				printf("\n\tenter your choice ");
				makeborder();
				scanf("%d",&cho);fprintf(logf,"\nchoice %d selected from scientific calculator module ",cho);clean();
